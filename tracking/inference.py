...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
            Loop on all the legalPositions
                True distance -> B
                Position (State) -> A
                P(A|B) = P(B|A) * P(A)
                allPossible[State] = emissionModel[TD] * self.beliefs
                P(A) -> self.beliefs
                Normalizes allpossible -> self.beliefs
        """
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Loop on all position -> pos
        #   newPosDist = self.getPositionDistribution() -> distribution
        #   Loop through newPosDist -> newPos,prob
        #       allPossible[newPos] += self.beliefs[pos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        numOfParticles = 0
        while numOfParticles < self.numParticles:
            for position in self.legalPositions:
                if numOfParticles < self.numParticles:
                    numOfParticles += 1
        ...
        if noisyDistance == None:
            ...
            if allPossible.totalCount() == 0:
                ...
                for i in range(self.numParticles):
                    newParticleList.append(util.sample(allPossible))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        particlePositions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(particlePositions)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                posDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
