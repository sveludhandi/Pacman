...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhosts:
            if (i):
                numLivingGhosts += 1
        for index in range(numLivingGhosts):
            currGhostPos = None
            ghostProb = 0
            ghostDistribution = livingGhostPositionDistributions[index]
            for ghostDistPos in ghostDistribution.keys():
                if ghostDistribution[ghostDistPos] > ghostProb:
                    currGhostPos = ghostDistPos
                    ghostProb = ghostDistribution[ghostDistPos]
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            ghostDist = self.distancer.getDistance(nextPos, currGhostPos)
